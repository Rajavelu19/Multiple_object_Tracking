...
def initialize_deepsort():
    ...
    cfg_deep = get_config()
    cfg_deep.merge_from_file("C:\\Users\\rajav\\Downloads\\Course_project\\YOLOv9-DeepSORT-Object-Tracking\\deep_sort_pytorch\\configs\\deep_sort.yaml")
...
def colorLabels(classid):
    if classid == 0: #person
        ...
        color = (0,149,255)
    elif classid == 24: #Backpack
        color = (0,100,250)
    else:
        color = (200, 100,0)
    return tuple(color)

def draw_boxes(frame, bbox_xyxy, draw_trails, s,identities=None, categories=None, offset=(0,0)):
    ...
    for i, box in enumerate(bbox_xyxy):
        ...
        data.append({
            'object id': id,
            'object name': className[cat],
            'bounding box': (y2 - y1, x2 - x1),  # Height, Width tuple
            'frame_num/Total_frame':(s[10:18])
        })
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                if len(outputs) > 0:
                    ...
                    object_id = outputs[:, -1]
                    draw_boxes(ims, bbox_xyxy, draw_trails,s, identities, object_id,)
...
#     return int(path.stem.split('_')[-1]) if '_' in path.stem else None
...
# Display the DataFrame
    ...
    csv_path = 'C:\\Users\\rajav\\Downloads\\Course_project\\YOLOv9-DeepSORT-Object-Tracking\\data.csv'
